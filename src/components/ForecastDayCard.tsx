...
export function ForecastDayCard({ entries }: Props) {
  ...
  const precipValues = entries.map((e) => e.precipitation).filter((p): p is number => p != null)
  const maxPrecipitation = precipValues.length > 0 ? Math.max(...precipValues) : 0
  ...
  const status = getActionStatus(minVisibility, maxWindSpeed, maxPrecipitation, maxWaveHeight)
  const card = (
      ...
      <CardContent>
        <div className="divide-y divide-border">
          ...
          <div className="flex items-center justify-between py-1.5">
            ...
            <span className={cn("font-medium", precipStyle(maxPrecipitation))}>
              {maxPrecipitation.toFixed(2)} {units.precipitation.substring(0, 2)}
            </span>
          </div>
          <div className="flex items-center justify-between py-1.5">
